var app = new Vue({
  ...
  mounted() {
    ...
    axios.get('http://192.168.1.34:3000/api/stops').then(response => this.stopsList = response.data);
  },
  ...
  methods: {
    selectStop(id) {
      this.stop = undefined;
      this.loading = true;
      axios.get('http://192.168.1.34:3000/api/stops/' + id).then(response => {
        this.loading = false;
    ...
    saveStop() {
      ...
      if (!this.stopSaved) this.stopSaved = {
        "Paradas": []
      };
      this.stopSaved.Paradas.push({
        "NumParada": this.stopId,
        "NombreParada": this.stop.NombreParada
      });
